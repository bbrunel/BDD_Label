...
z <- t %>% 
  filter(documentLabel == "https://www.cnis.fr/wp-content/uploads/2022/01/AC_2021_Insee_RNIPP.pdf") %>% 
  filter(duplicated(documentLabel))
t <- t %>% filter(!(intitule %in% z$intitule))
